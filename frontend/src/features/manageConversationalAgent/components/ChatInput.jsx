...
export default function ChatInput({ onSendMessage, onSelectSuggestion, suggestions }) {
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      ...
      if (containerRef.current && !containerRef.current.contains(e.target)) {
        ...
        if (contextWrapperRef.current && !contextWrapperRef.current.contains(e.target)) {
          setContextOpen(false);
        }
  ...
  return (
    <div className="w-full bg-gradient-to-t from-background via-background to-transparent pb-6 pt-8 px-4 sticky bottom-0 z-20">
      <div className="max-w-3xl mx-auto flex flex-col gap-3" ref={containerRef}>
        ...
        <div className={`relative group flex flex-col bg-white border rounded-3xl shadow-sm transition-all duration-500 ease-out overflow-visible ${
          isExpanded ? 'border-[var(--md-primary-container)]/30 shadow-md' : 'border-slate-200'
        }`}>
          ...
          >
            <div className={isExpanded ? 'overflow-visible' : 'overflow-hidden'}>
              <div className={`flex flex-col gap-3 px-3 pt-4 pb-2 border-b border-slate-100/50 transition-opacity duration-300 ${isExpanded ? 'opacity-100' : 'opacity-0'}`}>
                
                {/* Context Selector */}
                <div className="relative" ref={contextWrapperRef}>
                  <button 
                    onClick={(e) => { e.preventDefault(); setContextOpen(!contextOpen); }}
                    type="button"
                  ...
                  {contextOpen && isExpanded && (
                    <div className="absolute bottom-full left-0 mb-2 w-56 bg-white border border-slate-200 rounded-xl shadow-xl z-50 p-2 animate-fade-in [animation-duration:150ms] origin-bottom-left">
                      <div className="px-2 py-1.5 border-b border-slate-100 mb-1">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Select Area</p>
                      </div>
                      <div className="max-h-48 overflow-y-auto pr-1">
                        {CONTEXT_PAGES.map(page => {
                          const isSelected = selectedContext.find(p => p.id === page.id);
                          return (
                            <button
                              key={page.id}
                              ...
                              className={`w-full text-left px-3 py-1.5 text-xs rounded-md transition-colors flex items-center justify-between ${
                                isSelected ? 'bg-[var(--md-primary-container)]/10 text-[var(--md-primary-container)] font-bold' : 'text-slate-600 hover:bg-slate-50'
                              }`}
                            >
                              {page.label}
          ...
          <div className={`flex gap-2 p-2 px-3 transition-all duration-300 ${isExpanded ? 'items-end' : 'items-center'}`}>
            {!isExpanded && (
               <button 
                type="button"
            ...
            <textarea
              ...
              className={`flex-1 bg-transparent border-none focus:ring-0 text-sm font-medium text-slate-700 resize-none outline-none transition-all duration-300 ${
                isExpanded ? 'min-h-[44px] py-3 px-2' : 'min-h-[24px] py-1.5 px-2'
              }`}
          ...
          >
            <div className={isExpanded ? 'overflow-visible' : 'overflow-hidden'}>
              <div className={`flex items-center justify-between px-3 pb-3 pt-1 transition-opacity duration-300 ${isExpanded ? 'opacity-100' : 'opacity-0'}`}>
                <div className="relative" ref={modeWrapperRef}>
                  <button 
                    onClick={(e) => { e.preventDefault(); setModeOpen(!modeOpen); }}
                    type="button"
                  ...
                  >
                    <span className="material-symbols-outlined text-[16px] text-[var(--md-primary-container)]">{mode.icon}</span>
                    {mode.label}
                  ...
                  {modeOpen && isExpanded && (
                    <div className="absolute bottom-full left-0 mb-2 w-48 bg-white border border-slate-200 rounded-xl shadow-lg z-50 p-1.5 animate-fade-in [animation-duration:150ms] origin-bottom-left">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-2 py-1.5">Model Mode</p>
                      {AGENT_MODES.map(m => (
                        <button
                          key={m.id}
                          onClick={() => { setMode(m); setModeOpen(false); }}
                          className={`w-full text-left px-3 py-2 text-xs rounded-lg transition-colors flex items-center justify-between ${
                            mode.id === m.id ? 'bg-[var(--md-primary-container)] text-white font-bold' : 'text-slate-600 hover:bg-slate-50'
                          }`}
                        >
                          ...
                          {mode.id === m.id && <span className="material-symbols-outlined text-[14px]">check</span>}
                        </button>
                      ))}
                    </div>
                  )}
